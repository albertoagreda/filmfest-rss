...
function imprime() {
  ...
  for (movie of movies) {
    ...
    tagH2.innerHTML =
      ...
      filtro +
      parseInt(
        ordem == "anoCronologico" ? movie.anoCronologico : movie.anoLancamento
      );
...
function salvarFilme() {
  ...
  var validacao = validaDadosCadastro(
    nomeFilme,
    anoLancamento,
    imagem,
    anoCronologico,
  ...
  if (validacao.status) {
    var trailerLinkEdited = linkTrailer
      .substr(linkTrailer.indexOf("watch?v=", 0), 19)
      .substr(8, 11);
...
function validaDadosCadastro(
  ...
  let strSize = enderecoTrailer
    .substr(enderecoTrailer.indexOf("watch?v=", 0), 19)
    .substr(8, 11).length;
...
function tocarSom() {
  var audio = new Audio(
    "https://protettordelinks.com/wp-content/baixar/mario_moeda_efeito_sonoro_toquesengracadosmp3.com.mp3"
  );
  audio.play();
}
...
function printTrailer(e) {
  ...
  tagIFrame.setAttribute(
    "src",
    "https://www.youtube.com/embed/" + movie.dataset.linktrailer
  );
